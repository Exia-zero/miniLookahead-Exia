...
judge_client = openai.AsyncOpenAI(
    base_url=f"http://127.0.0.1:{args.judge_port}/v1", api_key="None", timeout=None
)
...
async def main():
    ...
    target_model = Targeter(
        args.model,
        eos_id=target_config["eos_id"],
        target_gpu_id=args.target_gpu_id,
        enable_n_gram=args.enable_n_gram,
        vllm_config={
            "force_eager": False,
    ...
    if args.use_spec:
        ...
        draft_model = Drafter(
            args.draft_model,
            eos_id=draft_config["eos_id"],
            draft_gpu_id=args.draft_gpu_id,
            enable_n_gram=args.enable_n_gram,
            vllm_config={
                "force_eager": False,
                "num_speculative_tokens": args.num_speculative_tokens,
        ...
        assert (
            target_config["name"] == draft_config["name"]
    ...
    if args.use_spec:
        ...
        print("Baseline mode: draft model is not loaded.")
