...
async def text_accept(response1, response2, last_ending, judge_client, judge_model):
    ...
    if (
        response1.replace(" ", "").replace("\n", "").lower()
        == response2.replace(" ", "").replace("\n", "").lower()
    ):
        return True
    ...
    response = await get_model_response(
        prompt, client=judge_client, model=judge_model, max_tokens=1
    )
...
async def accept_func(response1, response2, last_ending, judge_client, judge_model):
    return await text_accept(
        response1, response2, last_ending, judge_client, judge_model
    )
...
async def run_problem(
    ...
    if use_spec:
        if draft_model is None or draft_tokenizer is None or draft_config is None:
            raise ValueError("use_spec=True requires draft_model, draft_tokenizer, and draft_config.")
    ...
    if use_spec:
        root = TreeNode(
            prefix=target_prompt,
            prefix_token_ids=target_token_ids,
            draft_prefix_token_ids=draft_token_ids,  # draft_prefix=draft_prompt, draft_prefix_token_ids=draft_token_ids, \
            width=width,
            idx=0,
            depth=1,
            drafter=draft_model,
            targeter=target_model,
            empty=True,
            max_depth=max_depth,
            target_config=target_config,
            draft_config=draft_config,
            qid=question_id,
            ignore_half_sentence=ignore_half_sentence,
            accept_func=accept_func,
            judge_client=judge_client,
            draft2target=draft2target,
            target2draft=target2draft,
            judge_model=target_config["judge_model"],
        )
        ...
        while True:
            ...
            while root.main_data is not None and root.check_judge_children():
                ...
                if any(
                    eos_id in root.main_data["i"] for eos_id in target_config["eos_id"]
                ) or any(
                    eos_id in root.drafter_data["i"]
                    for eos_id in draft_config["eos_id"]
                ):
                    print(
                        "Found EOS in main data:",
                        root.depth,
                        root.idx,
                        root.main_data,
                        root.drafter_data,
                ...
                for cid, child in enumerate(children):
                    ...
                    if child.drafter_data is not None:
                        accepted = child.drafter_data[
                            "j"
                        ].result()  # accept(main_data[0], child.drafter_data[0])
                        accept_records.append(
                            {
                                "decision_id": len(accept_records),
                                "parent_depth": root.depth,
                                "parent_idx": root.idx,
                                "candidate_depth": child.depth,
                                "candidate_idx": child.idx,
                                "accepted": bool(accepted),
                                "status": "accepted" if accepted else "rejected",
                                "target_step": root.main_data["t"],
                                "draft_step": child.drafter_data["t"],
                                "target_num_tokens": root.main_data["n"],
                                "draft_num_tokens": child.drafter_data["n"],
                                "target_finish_reason": root.main_data["r"],
                                "draft_finish_reason": child.drafter_data["r"],
                                "target_stop_reason": root.main_data["s"],
                                "draft_stop_reason": child.drafter_data["s"],
                        ...
                        if accepted:
                            print(
                                ...
                                child.depth,
                                child.idx,
                                [root.main_data["t"], child.drafter_data["t"]],
                            )
                            accepted_id = cid
                            break
                        else:
                ...
                if accepted_id != -1:
                    ...
                    print(
                        ...
                        root.idx,
                        root.main_data,
                        [root.drafter_data, root.main_data],
                    )
                    ...
                    root = TreeNode(
                        ...
                        + root.drafter_data["t"]
                        + root.main_data["t"],
                        prefix_token_ids=root.prefix_token_ids
                        + root.drafter_data["ti"]
                        + root.main_data["i"],
                        draft_prefix_token_ids=root.draft_prefix_token_ids
                        + root.drafter_data["i"]
                        + root.main_data["di"],
                        width=root.width,
                        idx=root.idx,
                        ...
                        targeter=root.targeter,
                        empty=True,
                        max_depth=root.max_depth,
                        generated_tokens=root.generated_tokens
                        + root.drafter_data["n"]
                        + root.main_data["n"],
                        target_config=target_config,
                        draft_config=draft_config,
                    ...
                    print(
                        "Failed to find a child, creating a new root...",
                        root.depth,
                        root.idx,
                        root.main_data,
                    )
        ...
        full_tokens = (
            root.main.prefix_token_ids + root.main_data["i"]
    ...
    result = {
        ...
        "generation_text": target_tokenizer.decode(
            generation_tokens, skip_special_tokens=False
        ),
        "full_text": full_text,
